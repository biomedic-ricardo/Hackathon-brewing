...
##datos normalizados(lat,lon) y nuevo atributo volumen total (frecuenca*volumen)
datos_clust <- datos
...
```
Asignación de los datos a su respectivo cluster y se obtiene el volumen total del número de elementos de cada cluster.

Se obtiene la suma de volumenes totales de cada cluster y se obtiene el promedio para los 6 clusters (sin considerar frecuencia)
```{r}
print(datos_clust)
for( i in 1:6){
...
```{r}
#probablemente borrar en el futuro
...
df_vol <- datos_clust$Vol_Total  #volumen con frecuencia y se asigna a un nuevo data frame
df_vol <-  scale(df_vol ,center = TRUE, scale = TRUE)  #se normaliza volumen total (vol_entrega*frecuencia)
print(df_vol) #aquí solo utiliza el atributo de volumen total normalizado

k2_dist <- kmeans(df_vol, centers = 6, nstart = 25) #al volumen le hace un kmeans, este kmeans se hace en paralelo, este es de volumen total y se guarda en k2_dist
#str(k2_dist)

datos_clust$cluster_vol_total <- k2_dist$cluster
#actualizamos el dataframe con la asignación de clusters obtenidas de volumen total

#fviz_cluster(k2_dist, data = df_long_normalized) #Visualizamos con 2 clusters

print(k2_centers) #k2_centers representa los centroides de cada cluster, utilizando exclusivamente lat y long
df_cluster_original <- k2_centers  


df_cluster_original$cluster <- rownames(df_cluster_original)

print(df_cluster_original) #se agrega la etiqueta del cluster al que corresponde cada ubicación
...
print(cluster1_x)  #cluster1_x y cluster1_y son lat1 y lon1 respectivamente
print(cluster1_y)
...
} #respecto a la latitud y a la longitud, está obteniendo los valores para distancia_test
